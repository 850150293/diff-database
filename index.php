...
echo $dbArr['db1']['database'].'和'.$dbArr['db2']['database'].'有'.count($arr_diff_table).'个表不同。</br>';
//print_r($table2);

$commonTable = $db->getCommonArray($table1['db'],$table2['db']);
echo $dbArr['db1']['database'].'和'.$dbArr['db2']['database'].'有'.count($commonTable).'个表相同。</br>';
        ...
        <tbody><tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    $table1Row = $db->getTableRow($dbArr['db1'],$value);
                    $table2Row = $db->getTableRow($dbArr['db2'],$value);
                    if (!empty($table1Row) && !empty($table2Row)) {
                        $tableDiffRow = $db->getDiffField($table1Row, $table2Row);
                        echo isset($tableDiffRow) ? serialize($tableDiffRow) : 'error';
                    } else {
                        echo '这个需要自己手动对比！';
                    }
